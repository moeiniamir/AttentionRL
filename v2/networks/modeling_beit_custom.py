...
class BeitSelfAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs, # ! amir
        ...
        # ! amir: remove masked tokens here by setting their attention scores to -1e9
        if kwargs['bool_masked_pos'] is not None:
            bool_masked_pos = kwargs['bool_masked_pos']
            # bool_maske_pos.shape = (batch_size, seq_len)
            # attention_scores.shape = (batch_size, num_heads, seq_len, seq_len)
            attention_scores = torch.where(bool_masked_pos.unsqueeze(-1).unsqueeze(1), -1e9, attention_scores)
            attention_scores = torch.where(bool_masked_pos.unsqueeze(1).unsqueeze(1), -1e9, attention_scores)
        # !
...
class BeitModel(BeitPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs, # ! amir
        ...
        # ! amir
        # add a false at index 0 of bool_masked_pos to account for the [CLS] token
        if 'remove_masked' in kwargs and kwargs['remove_masked']:
            cls_bool_masked_pos = nn.functional.pad(bool_masked_pos, (1, 0), value=False)
        else:
            cls_bool_masked_pos = None
        # !
        ...
        encoder_outputs = self.encoder(
            ...
            bool_masked_pos=cls_bool_masked_pos, # ! amir
        )
