...
class ViTMAEEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #!:
        self.n_last_positions = config.n_last_positions
        if self.n_last_positions > 0:
            self.order_embeddings = nn.Parameter(
                torch.zeros(1, self.n_last_positions, config.hidden_size), requires_grad=True
            )
        #!
...
class ViTMAESelfAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs #! 
        ...
        attention_scores = torch.where(kmask.unsqueeze(-1).unsqueeze(1), attention_scores, -1e9)
        attention_scores = torch.where(kmask.unsqueeze(1).unsqueeze(1), attention_scores, -1e9)
        #!
