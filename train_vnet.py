...
for epoch in range(n_epochs):
    ...
    for mode in ['train', 'val']:
        ...
        for item in tqdm(dataloaders[mode]):
            ...
            with torch.no_grad():
                gen_caps, gen_attn = GenerateCaptionsWithBeamSearch(policy_network, cap_in, step_features)
            gen_caps = torch.stack(gen_caps)[0].to(device)
            gen_attn = torch.stack(gen_attn)[0].to(device)
            rand_attn = get_random_level(gen_attn)
            full_idx = torch.sum(gen_attn, dim=-1)
            idx = torch.sum(rand_attn, dim=-1)
            # for j in range(len(gen_caps)):
            #     print(id2string(gen_caps[j, :idx[j]].cpu().numpy(), tokenizer), " : ", id2string(cap_in[j], tokenizer))
            # print(id2string(gen_caps[:, :idx].cpu().numpy(), tokenizer, batch=True))
            reward = []
            for j in range(len(gen_caps)):
                reward.append(cap_reward.cider(all_captions[j], id2string(gen_caps[j, :full_idx[j]].cpu().numpy(), tokenizer)))
            reward = torch.tensor(reward, dtype=torch.float32).to(device)

            with torch.set_grad_enabled(mode == 'train'):
                output = value_network(gen_caps, rand_attn, features).squeeze()
        ...
        if mode == 'val' and total_loss < bestLoss:
            bestLoss = total_loss
            torch.save(value_network.state_dict(), "Models/value_network.pt")
